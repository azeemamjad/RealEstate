...
def fetch_data_from_redfin(
    ...
    # price
    if price_max:
        params.update({'max_price': f'{int(price_max)}'})
    if price_min:
        params.update({'min_price': f'{int(price_min)}'})

    # acres
    if lot_size_min:
        params.update({'min_parcel_size': f'{int(lot_size_min)}'})
    if lot_size_max:
        params.update({'max_parcel_size': f'{int(lot_size_max)}'})
    region_data = get_regional_data(search_term)
            ...
            datum.update(
                {
                    ...
                    'county': f'{get_county_by_city(addressZipcode=home.get("zip", ""), cities_data=cities_data)}',
